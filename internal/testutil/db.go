...
// ClassicSchema is the br (beads_rust) schema: same as Schema but without the
// seven GasTown agent columns (hook_bead, role_bead, agent_state, last_activity,
// role_type, rig, mol_type).
const ClassicSchema = `
CREATE TABLE issues (
	id TEXT PRIMARY KEY,
	title TEXT NOT NULL,
...
// NewClassicTestDB creates an in-memory SQLite database with the classic (br) schema,
// which omits the seven GasTown agent columns.
func NewClassicTestDB(t *testing.T) *sql.DB {
	t.Helper()
	db, err := sql.Open("sqlite3", ":memory:")
	require.NoError(t, err)
	_, err = db.Exec(ClassicSchema)
	require.NoError(t, err)
	return db
}
